...
class _SalonMenServiceScreenState extends State<SalonMenServiceScreen> {
  final ScrollController _scrollController = ScrollController();
  final MaleSalonController controller = Get.put(MaleSalonController());
  ...
  static const String SOURCE_PAGE = 'salon_men_services';
  static const String SOURCE_TITLE = 'Salon for Men';

  final Map<String, GlobalKey> _categoryKeys = {
    'Haircut & Styling': GlobalKey(),
    'Beard Grooming': GlobalKey(),
    'Facials & Cleanup': GlobalKey(),
    'Detan & Brightening': GlobalKey(),
    'Head & Shoulder Massage': GlobalKey(),
    'Manicure & Pedicure': GlobalKey(),
  };

  final Map<String, String> _serviceToCategory = {
    'classic_haircut': 'Haircut & Styling',
    'hair_styling': 'Haircut & Styling',
    'beard_trim_shape': 'Beard Grooming',
    'beard_styling': 'Beard Grooming',
    'charcoal_facial': 'Facials & Cleanup',
    'fruit_cleanup': 'Facials & Cleanup',
  };

  final Map<String, String> _categoryGridToSection = {
    'Haircut & Styling': 'Haircut & Styling',
    'Beard Grooming': 'Beard Grooming',
    'Facials & Cleanup': 'Facials & Cleanup',
    'Detan & Brightening': 'Facials & Cleanup',
    'Head & Shoulder Massage': 'Haircut & Styling',
    'Manicure & Pedicure': 'Beard Grooming',
  };

  @override
  void initState() {
    super.initState();
    print('🟢 SalonMenServiceScreen initialized with scrollToServiceId: ${widget.scrollToServiceId}');
  ...
  Widget _buildCategoryGrid(double scaleFactor) {
    final categories = [
      {'title': 'Haircut & Styling', 'image': 'assets/ms1.webp'},
      {'title': 'Beard Grooming', 'image': 'assets/ms2.webp'},
      {'title': 'Facials & Cleanup', 'image': 'assets/ms3.webp'},
      {'title': 'Detan & Brightening', 'image': 'assets/ms4.webp'},
      {'title': 'Head & Shoulder Massage', 'image': 'assets/ms5.webp'},
      {'title': 'Manicure & Pedicure', 'image': 'assets/ms6.webp'},
    ];
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: controller.groupedServices.entries.map((entry) {
            ...
            final List<MaleSalonService> services = entry.value;
            return Container(
  ...
  Widget _buildServiceCard(MaleSalonService service, double scaleFactor, String category) {
    if (category == 'Facials & Cleanup') {
      return _buildFacialCard(service, scaleFactor);
    } else {
      return _buildRegularServiceCard(service, scaleFactor);
    }
  }

  Widget _buildRegularServiceCard(MaleSalonService service, double scaleFactor) {
    return Container(
  ...
  Widget _buildFacialCard(MaleSalonService service, double scaleFactor) {
    return Container(
      margin: EdgeInsets.only(bottom: 16.r * scaleFactor),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  Widget _buildQuantitySelector(MaleSalonService service, double scaleFactor) {
    return Obx(() {
