...
class _RescheduleSummaryScreenState extends State<RescheduleSummaryScreen> {
  ...
  Future<void> _startWizardIfNeeded() async {
    ...
    // Only prompt for date if we successfully got an address, otherwise user might want to pick address first
    if ((addressId ?? "").isNotEmpty) {
       await _editDateAndTime();
    }
    
    if (dayToken.trim().isEmpty || preferredTime == null) return;
...
Future<void> _editSaathi() async {
    if ((addressId ?? "").isEmpty) {
      Get.snackbar("Address required", "Please select an address first");
      return;
    }
    if (dayToken.trim().isEmpty) {
      Get.snackbar("Day required", "Please select a scheduled day first");
      return;
    }
    if (preferredTime == null) {
      Get.snackbar("Time required", "Please select a time slot first");
      return;
    }
    if ((locationId ?? "").isEmpty) {
      Get.snackbar("Location missing", "Could not resolve location for address");
      return;
    }
  ...
  Future<void> _submitWithReason() async {
    ...
    try {
      ...
      final ok = await bookingController.rescheduleBookingTyped(
        ...
        addressId: addressId!,
        preferred: slot,
        rescheduleReason: _pickedReason,
      );
      ...
      if (ok) {
        ...
        final msg = bookingController.error.value.isNotEmpty
      ...
      body: SafeArea(
        child: Stack(
          children: [
            Column(
              children: [
                ChayanHeader(title: "Reschedule", onBack: () => Navigator.pop(context)),
                Expanded(
                    ...
                    child: Column(
                      ...
                      children: [
                        _InfoCardCompact(
                          ...
                          onEditAddress: _editAddress, // Calls the popup
                        ...
                        ..._reasons.map((r) => _ReasonTileCompact(
                              ...
                              onTap: () => setState(() => _pickedReason = r),
                            )),
